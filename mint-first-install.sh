...
sudo add-apt-repository ppa:apandada1/typhoon -y
sudo add-apt-repository ppa:apandada1/quickbib -y
...
sudo apt install htop foliate octave quickbib wike typhoon xournalpp marker blanket gimp inkscape remmina cheese gnome-video-effects-frei0r gnome-video-effects-extra simplescreenrecorder ffmpeg pdfarranger micro python-is-python3 python3-numpy python3-scipy mypaint jupyter-notebook github-desktop cpufetch plank ibus-avro gcolor3 python3-octave-kernel vlc okular texstudio texlive-full code stellarium kalzium gparted safeeyes telegram papirus-icon-theme fonts-junicode fonts-noto-color-emoji numbat com.github.phase1geo.annotator parabolic redshift chromium -y
